...
(function() {
  ...
  r.connect( { host: config.rethinkdb.host, port: config.rethinkdb.port }, function(err, conn) {
    ...
    connection = conn;
    r.db(config.rethinkdb.db).tableCreate('preguntaestudiante').run(conn, function(err, res) {
  ...
  exports.listarPreguntas = function(req, res) {
    try {
      r.db(config.rethinkdb.db).table('preguntaestudiante').run(connection, function (err, cursor) {
  ...
  exports.create = function(req, res) {
    try {
      ...
      r.db(config.rethinkdb.db).table('preguntaestudiante').insert(pregunta).
        run(connection, function (err, result) {
  ...
  exports.update = function(req, res) {
    try {
      var pregunta = req.query;
      r.db(config.rethinkdb.db).table('preguntaestudiante').get(pregunta.id).update(pregunta).
        run(connection, function (err, result) {
  ...
  exports.delete = function(req, res) {
    try {
      ...
      r.db(config.rethinkdb.db).table('preguntaestudiante').get(pregunta._id).delete().
        run(connection, function (err, result) {
